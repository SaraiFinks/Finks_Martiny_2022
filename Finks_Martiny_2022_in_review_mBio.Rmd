---
title: "Plasmid-Encoded Traits Vary across Environments"
...
filtered.plsdb <-
  read_excel(
    "/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/filtered_plsdb.xlsx"
  )
...
#Figure S2 - Taxonomic representation of plasmid host by environment in the PLSDB dataset.
...
#Figures 1 and S3 - plasmid size variation across environment, host-taxonomy, and host-taxonomy given environment.
...
#Figure S3A - Plasmid size distributions vary significantly by host taxonomy and environment
...
#Figure X - Plasmid coding density by environment (not included in final version)
...
all_mob <-
  read_excel(
    "/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/trait_analysis/all_mob.xlsx",
    sheet = "combined"
  )
...
#Figure S4 - The proportion of relaxases by plasmid host-taxonomy.
...
#Figure 2A - The proportion of MOB family relaxases by environment.
...
all_cogs <-
  read_excel(
    "/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/trait_analysis/all_cogs.xlsx",
    sheet = "cogs"
  )
...
#Figure 3 - Cumulative COG richness of plasmids by environment.
...
figure_3 <-
  rarecurve(
    cog_by_env_rare.t,
    step = 1000,
    xlab = "No. of plasmid genes observed",
...
#Figure 4 - Plasmid COG functions by environment
...
#Figure S6A - Plasmid COG categories by host phylum and environment
...
all_cazy <-
  read_excel(
    "/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/trait_analysis/all_cazy.xlsx",
    sheet = "cazymes"
  )
...
#Figure 5 - Plasmid resistance, carbon-degradation and nitrogen cycling traits by environment.
...
#Figure S6C - Plasmid carbon degrading traits by host phylum and environment.
...
all_nit <-
  read_excel(
    "/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/trait_analysis/all_nit.xlsx",
    sheet = "ncycl_v2"
  )
...
#Figure S6D - Plasmid nitrogen cycling traits by host phylum and environment.
...
all_args <-
  read_excel(
    "/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/trait_analysis/all_args.xlsx"
  )
...
#Figure S6B - Plasmid resistance traits by host phylum and environment.
...
```{r Assessing biases in the PLSDB dataset, echo=FALSE}
...
#For plot parameters and ideas, see https://rpubs.com/techanswers88/sankey-with-own-data-in-ggplot
...
figure_S1 <- ggplot(
  ...
  #geom_sankey_label(size = 0.5, hjust = -0.5) +
  geom_sankey_text(size = 0.5,
                   color = "black",
                   hjust = -0.5) +
  scale_fill_viridis_d() +
  scale_y_discrete(expand = c(0.01, 0.01)) +
  theme_sankey(base_size = 0.2) +
  theme(legend.position = "none") +
  theme(
...
```{r Quantifying plasmids counts: E. coli and K. pneumoniae, echo=FALSE}

ecoli <-
  read_excel(
    "~/Google_drive/uci_grad_school/my_projects/ch2/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/ecoli.xlsx"
  )

ecoli  <-
  as.data.frame(ecoli, col.names	=	TRUE,	row.names	=	TRUE)
ecoli[1:10 , 1:10]

ecoli.1 <- ecoli[-1]
row.names(ecoli.1) <- ecoli$UID_NUCCORE
sapply(ecoli.1, class)
ecoli.1[1:10, 1:10]

ecoli_by_env <-
  ecoli.1 %>% group_by(environment) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

ecoli_by_env_l <- pivot_longer(ecoli_by_env, cols = 1:6)
...
ecoli_env <-
  ...
  scale_fill_manual(values = c06) +
  theme_void() +
  theme(legend.position = "none") +
  geom_text(aes(y = value, label = name), color = "white", size = 6)

plot(ecoli_env) #figure not included in final version
...
ecoli_size_box <-
  ...
  scale_y_continuous(breaks = seq(0, 1000000, by = 10000),
                     expand = c(0, 0)) +
  coord_cartesian(clip = 'off') +
  geom_hline(yintercept = mean(ecoli.1$Length_NUCCORE),
             linetype = 2) + #Add horizontal line at base mean
  stat_compare_means(method = "kruskal.test", label.y = 3.5) + #Add global kw p-value
  stat_compare_means(
    label = "p.signif",
...
plot(ecoli_size_box) #figure not included in final version

#Testing whether whether human and non-human animals differ in their trait content
cogs_ecoli <-
  read_excel(
    "~/Google Drive/My Drive/uci_grad_school/my_projects/ch2/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/trait_analysis/all_cogs.xlsx",
    sheet = "ecoli"
  )

cogs_ecoli  <-
  as.data.frame(cogs_ecoli,
                col.names	=	TRUE,
                row.names	=	TRUE)

ecoli_cog_by_env <-
  cogs_ecoli  %>% group_by(environment) %>% group_by(COG20_CATEGORY_ACC1) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

ecoli_cog_by_env  <-
  as.data.frame(ecoli_cog_by_env, col.names	=	TRUE,	row.names	=	TRUE)

ecoli_cog_by_env  <-
  as.data.frame((ecoli_cog_by_env[-13, ]))

ecoli_cog_by_env  <-
  as.data.frame((ecoli_cog_by_env[, -7]))

ecoli_cog_by_env[is.na(ecoli_cog_by_env)] <- 0

row.names(ecoli_cog_by_env) = ecoli_cog_by_env[, 1]
ecoli_cog_by_env_traits <-
  as.data.frame((ecoli_cog_by_env[, -1]))

#test between the human and the non-human animal ecoli, lets subset this dataframe
ecoli_cog_host <-
  ecoli_cog_by_env_traits %>% dplyr::select(animal,
                                            human)
ecoli_cog_host  <-
  as.matrix(ecoli_cog_host)

#Test the likelihood of COG categories across environment
G.test.cog.ecoli.host <- GTest(ecoli_cog_host, correct = "none")
G.test.cog.ecoli.host

args_ecoli <-
  read_excel(
    "~/Google Drive/My Drive/uci_grad_school/my_projects/ch2/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/trait_analysis/all_args.xlsx",
    sheet = "ecoli"
  )

args_ecoli  <-
  as.data.frame(args_ecoli,
                col.names	=	TRUE,
                row.names	=	TRUE)

ecoli_args_by_env <-
  args_ecoli  %>% group_by(environment) %>% group_by(class) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

ecoli_args_by_env  <-
  as.data.frame(ecoli_args_by_env,
                col.names	=	TRUE,
                row.names	=	TRUE)

ecoli_args_by_env  <-
  as.data.frame((ecoli_args_by_env[-16, ]))

ecoli_args_by_env  <-
  as.data.frame((ecoli_args_by_env[, -6]))

ecoli_args_by_env[is.na(ecoli_args_by_env)] <- 0

row.names(ecoli_args_by_env) = ecoli_args_by_env[, 1]
ecoli_args_by_env_traits <-
  as.data.frame((ecoli_args_by_env[, -1]))

#So we want to test just between the human and the non-human animal ecoli, lets subset this dataframe
ecoli_args_host <-
  ecoli_args_by_env_traits %>% dplyr::select(animal,
                                             human)

ecoli_args_host  <-
  as.matrix(ecoli_args_host)

#Test the likelihood of ARGs by classes across environment
G.test.args.ecoli.host <- GTest(ecoli_args_host, correct = "none")
G.test.args.ecoli.host

cazys_ecoli <-
  read_excel(
    "~/Google Drive/My Drive/uci_grad_school/my_projects/ch2/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/trait_analysis/all_cazy.xlsx",
    sheet = "ecoli"
  )

cazys_ecoli  <-
  as.data.frame(cazys_ecoli,
                col.names	=	TRUE,
                row.names	=	TRUE)

ecoli_cazys_by_env <-
  cazys_ecoli  %>% group_by(environment) %>% group_by(cazy_domain1) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

ecoli_cazys_by_env  <-
  as.data.frame(ecoli_cazys_by_env,
                col.names	=	TRUE,
                row.names	=	TRUE)

ecoli_cazys_by_env  <-
  as.data.frame((ecoli_cazys_by_env[-25, ]))

ecoli_cazys_by_env[is.na(ecoli_cazys_by_env)] <- 0

row.names(ecoli_cazys_by_env) = ecoli_cazys_by_env[, 1]
ecoli_cazys_by_env_traits <-
  as.data.frame((ecoli_cazys_by_env[, -1]))

#So we want to test just between the human and the non-human animal ecoli, lets subset this dataframe
ecoli_cazys_host <-
  ecoli_cazys_by_env_traits %>% dplyr::select(animal,
                                              human)

ecoli_cazys_host  <-
  as.matrix(ecoli_cazys_host)

#Test the likelihood of CAZymes across environment
G.test.cazys.ecoli.host <- GTest(ecoli_cazys_host, correct = "none")
G.test.cazys.ecoli.host

nit_ecoli <-
  read_excel(
    "~/Google Drive/My Drive/uci_grad_school/my_projects/ch2/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/trait_analysis/all_nit.xlsx",
    sheet = "ecoli"
  )

nit_ecoli  <-
  as.data.frame(nit_ecoli,
                col.names	=	TRUE,
                row.names	=	TRUE)

ecoli_nit_by_env <-
  nit_ecoli  %>% group_by(environment) %>% group_by(gene_family) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

ecoli_nit_by_env  <-
  as.data.frame(ecoli_nit_by_env, col.names	=	TRUE,	row.names	=	TRUE)

ecoli_nit_by_env  <-
  as.data.frame((ecoli_nit_by_env[-15, ]))

ecoli_nit_by_env  <-
  as.data.frame((ecoli_nit_by_env[, -5]))

ecoli_nit_by_env[is.na(ecoli_nit_by_env)] <- 0

row.names(ecoli_nit_by_env) = ecoli_nit_by_env[, 1]
ecoli_nit_by_env_traits <-
  as.data.frame((ecoli_nit_by_env[, -1]))

#So we want to test just between the human and the non-human animal E. coli, lets subset this dataframe
ecoli_nit_host <-
  ecoli_nit_by_env_traits %>% dplyr::select(animal,
                                            human)

ecoli_nit_host  <-
  as.matrix(ecoli_nit_host)

#Test the likelihood of nitrogen gene families across environment
G.test.nit.ecoli.host <- GTest(ecoli_nit_host, correct = "none")
G.test.nit.ecoli.host
...
mob_ecoli  <-
  as.data.frame(mob_ecoli,
                col.names	=	TRUE,
                row.names	=	TRUE)

ecoli_mob_by_env <-
  mob_ecoli  %>% group_by(environment) %>% group_by(relaxase_family_name) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

ecoli_mob_by_env  <-
  as.data.frame(ecoli_mob_by_env, col.names	=	TRUE,	row.names	=	TRUE)

ecoli_mob_by_env  <-
  as.data.frame((ecoli_mob_by_env[-7, ]))

ecoli_mob_by_env[is.na(ecoli_mob_by_env)] <- 0

row.names(ecoli_mob_by_env) = ecoli_mob_by_env[, 1]
ecoli_mob_by_env_traits <-
  as.data.frame((ecoli_mob_by_env[, -1]))

#So we want to test just between the human and the non-human animal E. coli, lets subset this datafram
ecoli_mob_host <-
  ecoli_mob_by_env_traits %>% dplyr::select(animal,
                                            human)

ecoli_mob_host  <-
  as.matrix(ecoli_mob_host)

#Test the likelihood of mobility across environment
G.test.mob.ecoli.host <- GTest(ecoli_mob_host, correct = "none")
G.test.mob.ecoli.host

#When we consider just the sample size of the Klebsiella...
kpneum <-
  read_excel(
    "/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/kpneum.xlsx"
  )

kpneum  <-
  as.data.frame(kpneum, col.names	=	TRUE,	row.names	=	TRUE)
kpneum[1:10 , 1:10]

kpneum.1 <- kpneum[-1]
row.names(kpneum.1) <- kpneum$UID_NUCCORE
sapply(kpneum.1, class)
kpneum.1[1:10, 1:10]

kpneum_by_env <-
  kpneum.1 %>% group_by(environment) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

kpneum_by_env_l <- pivot_longer(kpneum_by_env, cols = 1:4)
...
plot(kpneum_env) #figure not included in final version

kpneum_size_box <-
  ggplot(
    data = kpneum.1,
    aes(
      x = fct_relevel(environment,
                      "human",
                      "animal",
                      "wastewater",
                      "marine"),
      y = Length_NUCCORE,
      fill = fct_relevel(environment,
                         "human",
                         "animal",
                         "wastewater",
                         "marine"),
      color = fct_relevel(environment,
                          "human",
                          "animal",
                          "wastewater",
                          "marine",)
  ...
  scale_y_continuous(breaks = seq(0, 1000000, by = 10000),
                     expand = c(0, 0)) +
  coord_cartesian(clip = 'off') +
  geom_hline(yintercept = mean(kpneum.1$Length_NUCCORE),
             linetype = 2) + #Add horizontal line at base mean
  stat_compare_means(method = "kruskal.test", label.y = 3.5) + #Add global kw p-value
  stat_compare_means(
    label = "p.signif",
...
plot(kpneum_size_box) #figure not included in final version

klebsiella <-
  read_excel(
    "~/Google_drive/uci_grad_school/my_projects/ch2/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/kpneum.xlsx",
    sheet = "klebsiella"
  )

klebsiella  <-
  as.data.frame(klebsiella, col.names	=	TRUE,	row.names	=	TRUE)
klebsiella[1:10 , 1:10]

klebsiella.1 <- klebsiella[-1]
row.names(klebsiella.1) <- klebsiella$UID_NUCCORE
sapply(klebsiella.1, class)
klebsiella.1[1:10, 1:10]

klebsiella_by_env <-
  klebsiella.1 %>% group_by(environment) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

klebsiella_by_env_l <- pivot_longer(klebsiella_by_env, cols = 1:7)
...
plot(klebsiella_env) #figure not included in final version
...
klebsiella_size_box <-
  ...
  scale_y_continuous(breaks = seq(0, 1000000, by = 10000),
                     expand = c(0, 0)) +
  coord_cartesian(clip = 'off') +
  geom_hline(yintercept = mean(klebsiella.1$Length_NUCCORE),
             linetype = 2) + #Add horizontal line at base mean
  stat_compare_means(method = "kruskal.test", label.y = 1000) + #Add global kw p-value
  stat_compare_means(
    label = "p.signif",
    label.y = 520000,
    method = "wilcox.test",
...
plot(klebsiella_size_box) #figure not included in final version

```

```{r Sensitivity analyses: Proteobacteria (Acinetobacteria, Escerichia, and Klebsiella removed) and Firmicutes (Staphylococcus removed), echo=FALSE}

cogs_prot <-
  read_excel(
    "/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/trait_analysis/all_cogs.xlsx",
    sheet = "proteobact_big3_removed"
  )

cogs_prot  <-
  as.data.frame(cogs_prot,
                col.names	=	TRUE,
                row.names	=	TRUE)

prot_cog_by_env <-
  cogs_prot  %>% group_by(environment) %>% group_by(COG20_CATEGORY_ACC1) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

prot_cog_by_env  <-
  as.data.frame(prot_cog_by_env, col.names	=	TRUE,	row.names	=	TRUE)

prot_cog_by_env  <-
  as.data.frame((prot_cog_by_env[-13, ]))

prot_cog_by_env  <-
  as.data.frame((prot_cog_by_env[, -9]))

prot_cog_by_env[is.na(prot_cog_by_env)] <- 0

row.names(prot_cog_by_env) = prot_cog_by_env[, 1]
prot_cog_by_env_traits <-
  as.data.frame((prot_cog_by_env[, -1]))

#Test the likelihood of COG categories across environment
G.test.cog.prot.host <-
  GTest(prot_cog_by_env_traits, correct = "none")
G.test.cog.prot.host
...
args_prot  <-
  as.data.frame(args_prot,
                col.names	=	TRUE,
                row.names	=	TRUE)

prot_args_by_env <-
  args_prot  %>% group_by(environment) %>% group_by(class) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

prot_args_by_env  <-
  as.data.frame(prot_args_by_env, col.names	=	TRUE,	row.names	=	TRUE)

prot_args_by_env  <-
  as.data.frame((prot_args_by_env[-23, ]))

prot_args_by_env  <-
  as.data.frame((prot_args_by_env[, -9]))

prot_args_by_env[is.na(prot_args_by_env)] <- 0

row.names(prot_args_by_env) = prot_args_by_env[, 1]
prot_args_by_env_traits <-
  as.data.frame((prot_args_by_env[, -1]))

#Test the likelihood of ARGs by classes across environment
G.test.args.prot.host <-
  GTest(prot_args_by_env_traits, correct = "none")
G.test.args.prot.host
...
cazys_prot  <-
  as.data.frame(cazys_prot,
                col.names	=	TRUE,
                row.names	=	TRUE)

prot_cazys_by_env <-
  cazys_prot  %>% group_by(environment) %>% group_by(cazy_domain1) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

prot_cazys_by_env  <-
  as.data.frame(prot_cazys_by_env,
                col.names	=	TRUE,
                row.names	=	TRUE)

prot_cazys_by_env  <-
  as.data.frame((prot_cazys_by_env[-77, ]))

prot_cazys_by_env  <-
  as.data.frame((prot_cazys_by_env[-79, ]))

prot_cazys_by_env  <-
  as.data.frame((prot_cazys_by_env[, -9]))

prot_cazys_by_env[is.na(prot_cazys_by_env)] <- 0

row.names(prot_cazys_by_env) = prot_cazys_by_env[, 1]
prot_cazys_by_env_traits <-
  as.data.frame((prot_cazys_by_env[, -1]))

#Test the likelihood of CAZymes across environment
G.test.cazys.prot.host <-
  GTest(prot_cazys_by_env_traits, correct = "none")
G.test.cazys.prot.host
...
nit_prot  <-
  as.data.frame(nit_prot,
                col.names	=	TRUE,
                row.names	=	TRUE)

prot_nit_by_env <-
  nit_prot  %>% group_by(environment) %>% group_by(gene_family) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

prot_nit_by_env  <-
  as.data.frame(prot_nit_by_env, col.names	=	TRUE,	row.names	=	TRUE)

prot_nit_by_env  <-
  as.data.frame((prot_nit_by_env[-35, ]))

prot_nit_by_env  <-
  as.data.frame((prot_nit_by_env[, -9]))

prot_nit_by_env[is.na(prot_nit_by_env)] <- 0

row.names(prot_nit_by_env) = prot_nit_by_env[, 1]
prot_nit_by_env_traits <-
  as.data.frame((prot_nit_by_env[, -1]))

#Test the likelihood of nitrogen gene families across environment
G.test.nit.prot.host <-
  GTest(prot_nit_by_env_traits, correct = "none")
G.test.nit.prot.host
...
mob_prot  <-
  as.data.frame(mob_prot,
                col.names	=	TRUE,
                row.names	=	TRUE)

prot_mob_by_env <-
  mob_prot  %>% group_by(environment) %>% group_by(relaxase_family_name) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

prot_mob_by_env  <-
  as.data.frame(prot_mob_by_env, col.names	=	TRUE,	row.names	=	TRUE)

prot_mob_by_env  <-
  as.data.frame((prot_mob_by_env[-(8:9), ]))

prot_mob_by_env  <-
  as.data.frame((prot_mob_by_env[, -9]))

prot_mob_by_env[is.na(prot_mob_by_env)] <- 0

row.names(prot_mob_by_env) = prot_mob_by_env[, 1]
prot_mob_by_env_traits <-
  as.data.frame((prot_mob_by_env[, -1]))

#Test the likelihood of mobility across environment
G.test.mob.prot.host <-
  GTest(prot_mob_by_env_traits, correct = "none")
G.test.mob.prot.host

cogs_firm <-
  read_excel(
    "/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/trait_analysis/all_cogs.xlsx",
    sheet = "firm_staph_removed"
  )

cogs_firm  <-
  as.data.frame(cogs_firm,
                col.names	=	TRUE,
                row.names	=	TRUE)

firm_cog_by_env <-
  cogs_firm  %>% group_by(environment) %>% group_by(COG20_CATEGORY_ACC1) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

firm_cog_by_env  <-
  as.data.frame(firm_cog_by_env, col.names	=	TRUE,	row.names	=	TRUE)

firm_cog_by_env  <-
  as.data.frame((firm_cog_by_env[-13, ]))

firm_cog_by_env  <-
  as.data.frame((firm_cog_by_env[-22, ]))

firm_cog_by_env  <-
  as.data.frame((firm_cog_by_env[, -9]))

firm_cog_by_env[is.na(firm_cog_by_env)] <- 0

row.names(firm_cog_by_env) = firm_cog_by_env[, 1]
firm_cog_by_env_traits <-
  as.data.frame((firm_cog_by_env[, -1]))

#Test the likelihood of COG categories across environment
G.test.cog.firm.host <-
  GTest(firm_cog_by_env_traits, correct = "none")
G.test.cog.firm.host
...
args_firm  <-
  as.data.frame(args_firm,
                col.names	=	TRUE,
                row.names	=	TRUE)

firm_args_by_env <-
  args_firm  %>% group_by(environment) %>% group_by(class) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

firm_args_by_env  <-
  as.data.frame(firm_args_by_env, col.names	=	TRUE,	row.names	=	TRUE)

firm_args_by_env  <-
  as.data.frame((firm_args_by_env[-12, ]))

firm_args_by_env  <-
  as.data.frame((firm_args_by_env[, -8]))

firm_args_by_env[is.na(firm_args_by_env)] <- 0

row.names(firm_args_by_env) = firm_args_by_env[, 1]
firm_args_by_env_traits <-
  as.data.frame((firm_args_by_env[, -1]))

#Test the likelihood of ARGs by classes across environment
G.test.args.firm.host <-
  GTest(firm_args_by_env_traits, correct = "none")
G.test.args.firm.host
...
cazys_firm  <-
  as.data.frame(cazys_firm,
                col.names	=	TRUE,
                row.names	=	TRUE)

firm_cazys_by_env <-
  cazys_firm  %>% group_by(environment) %>% group_by(cazy_domain1) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

firm_cazys_by_env  <-
  as.data.frame(firm_cazys_by_env,
                col.names	=	TRUE,
                row.names	=	TRUE)

firm_cazys_by_env  <-
  as.data.frame((firm_cazys_by_env[-51, ]))

firm_cazys_by_env[is.na(firm_cazys_by_env)] <- 0

row.names(firm_cazys_by_env) = firm_cazys_by_env[, 1]
firm_cazys_by_env_traits <-
  as.data.frame((firm_cazys_by_env[, -1]))

#Test the likelihood of CAZymes across environment
G.test.cazys.firm.host <-
  GTest(firm_cazys_by_env_traits, correct = "none")
G.test.cazys.firm.host
...
nit_firm  <-
  as.data.frame(nit_firm,
                col.names	=	TRUE,
                row.names	=	TRUE)

firm_nit_by_env <-
  nit_firm  %>% group_by(environment) %>% group_by(gene_family) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

firm_nit_by_env  <-
  as.data.frame(firm_nit_by_env, col.names	=	TRUE,	row.names	=	TRUE)

firm_nit_by_env  <-
  as.data.frame((firm_nit_by_env[-18, ]))

firm_nit_by_env  <-
  as.data.frame((firm_nit_by_env[, -9]))

firm_nit_by_env[is.na(firm_nit_by_env)] <- 0

row.names(firm_nit_by_env) = firm_nit_by_env[, 1]
firm_nit_by_env_traits <-
  as.data.frame((firm_nit_by_env[, -1]))

#Test the likelihood of nitrogen gene families across environment
G.test.nit.firm.host <-
  GTest(firm_nit_by_env_traits, correct = "none")
G.test.nit.firm.host
...
mob_firm  <-
  as.data.frame(mob_firm,
                col.names	=	TRUE,
                row.names	=	TRUE)

firm_mob_by_env <-
  mob_firm  %>% group_by(environment) %>% group_by(relaxase_family_name) %>% dplyr::count(environment) %>% pivot_wider(names_from = environment, values_from = n)

firm_mob_by_env  <-
  as.data.frame(firm_mob_by_env, col.names	=	TRUE,	row.names	=	TRUE)

firm_mob_by_env  <-
  as.data.frame((firm_mob_by_env[-(10), ]))

firm_mob_by_env[is.na(firm_mob_by_env)] <- 0

row.names(firm_mob_by_env) = firm_mob_by_env[, 1]
firm_mob_by_env_traits <-
  as.data.frame((firm_mob_by_env[, -1]))

#Test the likelihood of mobility across environment
G.test.mob.firm.host <-
  GTest(firm_mob_by_env_traits, correct = "none")
G.test.mob.firm.host

```

```{r Plasmid replicon types by environment and accessory traits, echo=FALSE}

#Adding a different grouping of replicon types, following Douarre et al., 2020 groupings.
rep_by_env_traits2 <-
  read_excel(
    "/plasmids__2020_06_29__v0.4.1-6-g8ad6422194/replicon_counts.xlsx",
    sheet = "Sheet2"
  )

rep_by_env_traits2  <-
  as.data.frame(rep_by_env_traits2,
                col.names	=	TRUE,
                row.names	=	TRUE)

rep_by_env_traits2.1 <- rep_by_env_traits2[-1]
row.names(rep_by_env_traits2.1) <- rep_by_env_traits2$replicon_type

G.test.rep.env <- GTest(rep_by_env_traits2.1, correct = "none")
G.test.rep.env

#Remove all rows who sum to less than 6 (i.e., remove rare rep)
rep_by_env_traits.m <-
  rep_by_env_traits2.1[rowSums(rep_by_env_traits2.1) >= 6,]
nrow(rep_by_env_traits.m)
rowSums(rep_by_env_traits.m)

colSums(rep_by_env_traits.m)
rowSums(rep_by_env_traits.m)
nrow(rep_by_env_traits.m)

rep_sum <- (934 + 3343 + 25 + 48 + 57 + 34 + 17)
rep_sum

rep_anim_hum_percentage <- ((934 + 3343) / rep_sum) * 100
rep_anim_hum_percentage

rep_other_env <- ((25 + 48 + 57 + 34 + 17) / rep_sum) * 100
rep_other_env

#write.xlsx(
#rep_by_env_traits.m,
#file = "rep_counts2.xlsx",
#sheetName = "Sheet1",
#col.names = TRUE,
#row.names = TRUE
#)
...
rep.env.stand <-
  sweep(rep_by_env_traits.m,
        2,
        colSums(rep_by_env_traits.m),
        `/`)
colSums(rep.env.stand) #all of the columns sums should add to 1

normalized.rep <-
  rowNorms(rep.env.stand,
           type = "z",
           center = TRUE,
           scale = TRUE)

rowSums(normalized.rep)

#write.xlsx(
#normalized.rep,
#file = "rep_categories_norm.xlsx",
#sheetName = "Sheet1",
#col.names = TRUE,
#row.names = TRUE
#)

#Then using the standardized matrix, first by env (columns) then by reps(rows) make a Euclidean distance matrix
dist_env_rep <- vegdist(t(normalized.rep), method = 'euclidean')
clusters.env.rep <- hclust(dist_env_rep, 'ward.D')

dist_rep <- vegdist(normalized.rep, method = 'euclidean')
clusters.rep <- hclust(dist_rep, 'ward.D')

figure_S5 <- heatmap.2(
  normalized.rep,
  Rowv = as.dendrogram(clusters.rep),
  Colv = as.dendrogram(clusters.env.rep),
  col = c(
    "#236B8E",
    "#33A1DE",
    "#87CEEB",
  ...
  sepwidth = c(0.0000001, 0.000001),
  sepcolor = "#FAF9F6",
  colsep = 1:ncol(normalized.rep),
  rowsep = 1:nrow(normalized.rep),
  cexRow = 0.5,
  cexCol = 0.5,
  lhei = c(2, 10),
  lmat = rbind(c(0, 3, 4), c(2, 1, 0)),
...
#Figure 2B - Plasmid replicons vary across environments 

#Remember to remove phyla with column sums equal to zero
apply(rep_by_env_traits2.1, 1, sum)
rep_by_env_traits2.1.t <- t(rep_by_env_traits2.1)
rowSums(rep_by_env_traits2.1.t)
rep_by_env_traits2.1.1 <-  
  decostand(rep_by_env_traits2.1.t, method = "total")
apply(rep_by_env_traits2.1.1, 1, sum)
rep_by_env_traits2.1.1 <- as.data.frame(rep_by_env_traits2.1.1)
rep_by_env_traits2.1.1$row_names <- row.names(rep_by_env_traits2.1.1)
rep_by_env_traits.4 <-  rep_by_env_traits2.1.1 %>%
  pivot_longer(!row_names, names_to = "rep_name", values_to = "rep_abundance")

rep_by_env_traits.4 <- as.data.frame(rep_by_env_traits.4)
rep_by_env_traits.4$rep_name[rep_by_env_traits.4$rep_abundance  < 0.01] <-
  rep_by_env_traits.4$rep_name["rep < 0.01"]

rep_by_env_traits.4$rep_name <-
  as.character(rep_by_env_traits.4$rep_name)

rep_by_env_traits.4$rep_name[is.na(rep_by_env_traits.4$rep_name)] <-
  "rep < 0.01"
rep_by_env_traits.4$rep_abundance <-
  as.numeric(rep_by_env_traits.4$rep_abundance)

kw.rep.env <-
  kruskal.test(rep_abundance ~ row_names,
               data = rep_by_env_traits.4)
kw.rep.env
...
figure_2B <-
  ggplot(rep_by_env_traits.4,
         aes(
           ...
           y = rep_abundance,
           fill = fct_reorder(rep_name, rep_abundance)
         )) +
  geom_bar(stat = "identity", position = "fill") +
  ...
  labs(fill = "rep_name") +
  xlab(label = "Environment") +
  ylab(label = "Proportion of replicon types") +
  guides(fill = guide_legend(title = "replicon types"))
